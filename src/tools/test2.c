...
get_loadaddress()
{
  ...
  load_address |= gethex2(2);
  load_address <<=8;
  load_address |= gethex2(4);
...
get_record()
{
   ...
   *(sbuffer+i) = 0 ;  /* put terminator */


  /* need line delay on terminal */
 /*  puts(sbuffer);  */




   byte_count = gethex2(0)-3;  /* only data record */

   address16bit = get_loadaddress();

   dptr = address16bit;
  for(x=0; x<byte_count; x++)
   {
    *(dptr+x) = gethex2((x*2)+6);
   }
  ...
  if(save_bcc != gethex2((x*2)+6)) bcc_error=1;
...
main()
{
   ...
   newline();
   puts("6809 MICROPROCESSOR KIT 2017");
